...
DESCRIPTION_OVERRIDES = {
    ...
    "linux system": "Inspect Linux host identity, CPU, memory, uptime, and resource limits",
    "linux storage": "Inspect Linux filesystem usage, mounts, inodes, and largest paths",
    "linux logs": "Read Linux journal, kernel, and file logs with bounded output",
    "linux services": "Inspect or safely operate systemd services",
    "linux network": "Inspect Linux interfaces, routes, DNS, and socket connections",
    "linux privilege": "Check command availability and non-interactive sudo authorization without executing it",
    ...
    "git operations": "Preview or execute Git synchronization, recovery, branch, remote administration, bisect, and maintenance operations safely",
    ...
    "podman connection": "Preview or execute Podman system connection operations for local and SSH remote services",
    "podman images": "List images stored by a local or remote Podman service",
    "podman inspect": "Inspect a Podman container, image, pod, volume, or network",
    "podman image-reference": "Validate a deterministic fully-qualified Podman registry image reference",
    "podman pull": "Pull a fully-qualified Docker Hub or GHCR image into a local or remote Podman store",
    "podman build": "Build an image from a Containerfile with qualified base images on a local or remote Podman service",
    "podman run": "Preview or execute a Podman container start with bastion-safe published ports",
