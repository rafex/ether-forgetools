...
Primary source (auto-generated by CI, includes descriptions):
    docs/catalog.json               → single merged file (preferred)

Fallback sources (used if catalog.json unavailable):
...
_CATALOG_URL  = f"{_RAW}/docs/catalog.json"        # primary — CI-generated, includes descriptions
_MANIFEST_URL = f"{_RAW}/releases/manifest.json"   # fallback
_STATUS_URL   = f"{_RAW}/docs/maven-central-status.json"  # fallback

# ── Local cache ───────────────────────────────────────────────────────────────

_CACHE_DIR     = Path.home() / ".cache" / "forgetools"
_CACHE_CATALOG = _CACHE_DIR / "ether-catalog.json"
...
def _load_catalog(force: bool = False) -> tuple[dict, dict]:
    ...
    Tries catalog.json first (CI-generated, all data merged + descriptions).
    Falls back to manifest.json + maven-central-status.json if unavailable.
    """
    _CACHE_DIR.mkdir(parents=True, exist_ok=True)

    # ── Primary: catalog.json ────────────────────────────────────────────────
    try:
        catalog: dict = _load_cached(_CACHE_CATALOG, _CATALOG_URL, force)
        manifest_by_name: dict[str, dict] = {m["name"]: m for m in catalog.get("modules", [])}
        # catalog.json already merges status; build a compatible status_by_name
        status_by_name: dict[str, dict] = {
            name: {
                "module":        name,
                "deployed":      m.get("deployed", False),
                "latestVersion": m.get("version", ""),
                "artifactUrl":   m.get("artifactUrl", ""),
            }
            for name, m in manifest_by_name.items()
        }
        # Inject deploy order into manifest_by_name meta
        manifest_by_name["__deployOrder__"] = catalog.get("deployOrder", [])  # type: ignore[assignment]
        return manifest_by_name, status_by_name
    except Exception:
        pass  # fall through to individual files

    # ── Fallback: manifest + status separately ────────────────────────────────
    manifest_raw: dict = _load_cached(_CACHE_MAN,  _MANIFEST_URL, force)
    status_raw:   list = _load_cached(_CACHE_STAT, _STATUS_URL,   force)
    manifest_by_name = {m["name"]: m for m in manifest_raw.get("modules", [])}
    manifest_by_name["__deployOrder__"] = manifest_raw.get("deployOrder", [])  # type: ignore[assignment]
...
def _merge(name: str, man: dict, stat: dict) -> dict:
    s = stat.get(name, {})
    # description: prefer catalog.json value, fallback to embedded dict
    desc = man.get("description") or _DESCRIPTIONS.get(name, "")
    return {
        "name":        name,
...
def _do_list(man: dict, stat: dict, deployed_only: bool) -> dict:
    modules = [
        ...
        and (not deployed_only or stat.get(name, {}).get("deployed", m.get("published")))
    ]
    return {
        "total":    len(modules),
        "modules":  modules,
...
def _do_order(man: dict, stat: dict) -> dict:
    order = man.get("__deployOrder__") or [k for k in man if not k.startswith("__")]
    return {
        "deploy_order": [
            {
                "position": i + 1,
