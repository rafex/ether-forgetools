"""forgetools.cargo.build — Run cargo build and parse JSON output."""
        ...
        try:
            ...
            if rc != 0:
                return ForgeResult.failure(
                    TOOL,
                    [f"cargo build failed with {data['errors']} error(s)"],
                    t.elapsed_ms,
                )
...
def _parse(output: str, rc: int) -> dict:
    ...
    for line in output.splitlines():
        ...
        fname = primary_span.get("file_name")
...
def _add_args(p: argparse.ArgumentParser) -> None:
    p.add_argument("--release", action="store_true", help="Build in release mode")
    p.add_argument("--package", default=None, help="Package to build")


if __name__ == "__main__":
    make_cli(TOOL, "Run cargo build", run, _add_args)
